...
vterm executes [VHS](https://github.com/charmbracelet/vhs)-style `.tape` scripts against a **real PTY**, models the screen with an offscreen terminal emulator ([avt](https://crates.io/crates/avt)), and rasterizes PNG/GIF output itself. It is a single static binary with zero runtime dependencies — no Chromium, no ttyd, no ffmpeg.
...
Type "echo hello from vterm"
Enter
Wait

Assert /hello from vterm/
...
```sh
vterm run hello.tape          # execute (also: `vterm hello.tape`)
vterm run --json hello.tape   # same, with a JSON report on stdout
vterm check hello.tape        # parse + validate only, no execution
echo 'Type "date"' | vterm run -   # read the tape from stdin
```
...
| 0    | success                                              |
| 1    | `Assert` failure                                     |
| 2    | parse/validation error (also from `vterm check`)     |
| 3    | `Wait` timeout                                       |
| 4    | runtime error (I/O, PTY, missing `Require` binary)   |

### `--json` run report

`vterm run --json` prints exactly one JSON object on stdout. Real output from the quickstart tape above (comments annotate; they are not part of the output):
...
```jsonc
{
  ...
  "commands": [                    // one record per executed command
    ...
    { "index": 1, "line": 3, "col": 1,
      "command": "Type echo hello from vterm", "status": "ok", "elapsed_ms": 1070 },
    ...
    { "index": 4, "line": 7, "col": 1,
      "command": "Assert Screen hello from vterm", "status": "ok", "elapsed_ms": 0,
      "detail": { "matched": true, "regex": "hello from vterm", "scope": "Screen" } },
...
`vterm check --json` prints `{"ok": bool, "commands": N, "errors": [{"line", "col", "message"}]}`.

### Failure forensics — always on

On **any** failure (assert, wait timeout, runtime error) vterm writes two files next to your outputs, no flags required:
...
- environment: `TERM=xterm-256color`, `PS1="> "` (matches the default `WaitPattern` `>$`), `PROMPT_COMMAND=`, `HISTFILE=`, `LANG=LC_ALL=C.UTF-8`, plus `VTERM=1` so scripts can detect they're under automation
...
The grammar is VHS's, with three extensions (`Assert`, `Capture`, `Output .cast`). Existing `.tape` files parse unchanged; `vterm check` flags anything vterm cannot execute, with `line:col` caret errors.
...
**Dropped** (rejected by `vterm check`, exit 2)
...
- `Set FontFamily` — vterm renders with an embedded JetBrains Mono; ignored with a warning
...
- **Mid-tape `Set` is restricted.** Terminal geometry is fixed once the shell spawns, so after the first action command only `TypingSpeed`, `WaitTimeout`, `WaitPattern`, `PlaybackSpeed`, and `Theme` may be `Set`; anything else is a `check`-time error (VHS silently ignores these — vterm refuses).
...
All in [`examples/`](examples/), all pass `vterm check`:
...
- [charmbracelet/vhs](https://github.com/charmbracelet/vhs) (MIT) — the tape language, defaults, and `themes.json`; vterm's parser is a faithful port of VHS's
- [asciinema](https://github.com/asciinema/asciinema) — the [avt](https://crates.io/crates/avt) terminal emulator and the PTY-handling patterns vterm's session engine is modeled on; `Output .cast` follows asciicast v3
